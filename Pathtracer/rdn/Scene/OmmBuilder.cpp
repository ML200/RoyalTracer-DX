...
void OmmBuilder::BakeAll(std::vector<MeshGPU>& meshes, const MaterialSoA& materials,
    ...
    for (size_t mi = 0; mi < meshes.size(); ++mi) {
        ...
        if (mesh.alphaTriCount == 0)
            continue;

        mesh.ommBake.triOmmIndices.resize(mesh.alphaTriCount);

        std::fill(mesh.ommBake.triOmmIndices.begin(), mesh.ommBake.triOmmIndices.end(),
        ...
        for (uint32_t t = 0; t < mesh.alphaTriCount; ++t) {
            ...
            const bool isThinGlass = matID < materials.thinGlass.size() && materials.thinGlass[matID] != 0u;
            const bool isTransmissive = materials.Kd[matID].w < 1.0f;
            if (isThinGlass || isTransmissive) {
                mesh.ommBake.triOmmIndices[t] = D3D12_RAYTRACING_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_UNKNOWN_OPAQUE;
                continue;
            }
    ...
    LOG(L"[OMM] " << globalGroups.size() << L" unique textures: " << texWithAlpha << L" need baking (" << totalAlphaTris
                  << L" tris), " << texSkipped << L" skipped (uniform alpha)");
    ...
    for (auto& [texID, group] : globalGroups) {
        ...
        std::unordered_map<uint64_t, uint32_t> vertRemap;
        ...
        ++bakeIdx;
        LOG(L"[OMM] [" << bakeIdx << L"/" << texWithAlpha << L"] Baking texID " << texID << L": " << group.tris.size()
                       << L" tris, " << img->width << L"x" << img->height << L" tex, " << (localIndices.size() / 3)
                       << L" unique tris");
        ...
        if (res != ommResult_SUCCESS) {
            LOG(L"[OMM] Bake failed (err=" << (int)res << L") texID=" << texID << L" tris=" << group.tris.size());
        ...
        for (auto& [meshIdx, positions] : meshTriPositions) {
            ...
            result.rawData.insert(result.rawData.end(), (const uint8_t*)desc->arrayData,
        ...
        if (desc->descArrayCount > 0)
            LOG(L"[OMM] texID " << texID << L": " << group.tris.size() << L" tris (across " << meshTriPositions.size()
                                << L" meshes), " << desc->descArrayCount << L" unique OMMs, "
    ...
    LOG(L"[OMM] BakeAll done: " << totalBakedOMMs << L" unique OMMs, " << (totalBytes / 1024) << L" KB, " << ms
                                << L" ms");
}
...
OmmGpuData OmmBuilder::BuildGPU(const OmmBakeResult& bake, ID3D12Device5* device, ID3D12GraphicsCommandList4* cmdList) {
    ...
    if (!bake.triOmmIndices.empty()) {
        UINT64 ibSize = bake.triOmmIndices.size() * sizeof(int32_t);
        auto desc = CD3DX12_RESOURCE_DESC::Buffer(ibSize);
        ThrowIfFailed(device->CreateCommittedResource(&nv_helpers_dx12::kUploadHeapProps, D3D12_HEAP_FLAG_NONE, &desc,
    ...
    {
        UINT64 sz = ROUND_UP(prebuildInfo.ScratchDataSizeInBytes, D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT);
        gpu._scratchBuffer =
            nv_helpers_dx12::CreateBuffer(device, sz, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
                                          D3D12_RESOURCE_STATE_COMMON, nv_helpers_dx12::kDefaultHeapProps);
    }
    {
        UINT64 sz =
            ROUND_UP(prebuildInfo.ResultDataMaxSizeInBytes, D3D12_RAYTRACING_OPACITY_MICROMAP_ARRAY_BYTE_ALIGNMENT);
        gpu.ommArray = nv_helpers_dx12::CreateBuffer(device, sz, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
