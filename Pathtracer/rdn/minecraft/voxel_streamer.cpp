...
void VoxelStreamer::light_snapshot(const planet::DVec3& sceneOrigin, std::vector<lt::TLASExtraLeaf>& out,
    ...
    for (uint32_t i = 0; i < (uint32_t)m_lightSlots.size(); ++i) {
        ...
        XMFLOAT3X3 normal33;
        lt::computeNormal33FromWorld(worldF, normal33);
        lt::SgCluster local;
        local.mean = {s.light.mean[0], s.light.mean[1], s.light.mean[2]};
        local.variance = s.light.variance;
        local.rbar = {s.light.rbar[0], s.light.rbar[1], s.light.rbar[2]};
        local.power = s.light.power;
        local.radius = s.light.radius;
        local.theta_o = std::acos(std::clamp(s.light.cosTheta, -1.0f, 1.0f));
        leaf.sg = lt::sgTransform(local, worldF, normal33, areaScale);
        leaf.power = leaf.sg.power;
        leaf.cone.axis = lt::sgAxis(leaf.sg.rbar);
        leaf.cone.theta_o = leaf.sg.theta_o;
...
void VoxelStreamer::run_job(const std::shared_ptr<MeshJob>& job) {
    ...
    if (g.light.valid()) {
        ...
        g.light.bmin[0] = root.mean.x - root.radius; g.light.bmin[1] = root.mean.y - root.radius; g.light.bmin[2] = root.mean.z - root.radius;
        g.light.bmax[0] = root.mean.x + root.radius; g.light.bmax[1] = root.mean.y + root.radius; g.light.bmax[2] = root.mean.z + root.radius;
        g.light.mean[0] = root.mean.x; g.light.mean[1] = root.mean.y; g.light.mean[2] = root.mean.z;
        g.light.variance = root.variance;
        g.light.rbar[0] = root.rbar.x; g.light.rbar[1] = root.rbar.y; g.light.rbar[2] = root.rbar.z;
        g.light.radius = root.radius;
        g.light.power = root.power;
        g.light.cosTheta = root.cosTheta_o;
    }
    job->gpu = g;
