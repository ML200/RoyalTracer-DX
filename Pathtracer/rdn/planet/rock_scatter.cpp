...
std::vector<RockMesh> generate_rock_variants(int count, int subdiv, uint32_t seed) {
    ...
    for (int v = 0; v < count; ++v) {
        ...
        for (size_t i = 0; i < dir.size(); ++i) {
            ...
            m.vertices[i].u = (float)(0.5 + std::atan2(p.z, p.x) * 0.15915494309);
            m.vertices[i].v = (float)(0.5 - std::asin(yy)        * 0.31830988618);
        }
    }
    return out;
}
...
bool RockScatter::update(const DVec3& camera_world, const IRockHeight& height) {
    ...
    if (m_have) {
        ...
        if (cosang < -1.0) cosang = -1.0;
        const double moved = std::acos(cosang) * m_cfg.planet.radius;
