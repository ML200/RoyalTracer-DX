...
std::string Version(const std::wstring& path) {
    ...
    UINT length = 0;
    if (!VerQueryValueW(data.data(), L"\\", (void**)&info, &length) || length < sizeof(*info))
        return "unknown";
    char version[64];
    snprintf(version, sizeof(version), "%u.%u.%u.%u", HIWORD(info->dwFileVersionMS), LOWORD(info->dwFileVersionMS),
...
void Transition(ID3D12GraphicsCommandList* cmd, ID3D12Resource* resource, D3D12_RESOURCE_STATES& state,
...
void DLSSNRManager::DiscoverRuntime() {
    ...
    m_runtimeFile.clear();
    m_status.runtimeSearched.clear();
    for (const auto& path : candidates) {
        m_status.runtimeSearched += Utf8(path) + "\n";
        DWORD attr = GetFileAttributesW(path.c_str());
        if (m_runtimeFile.empty() && attr != INVALID_FILE_ATTRIBUTES && !(attr & FILE_ATTRIBUTE_DIRECTORY))
            m_runtimeFile = path;
    }
    if (m_runtimeFile.empty()) {
        ...
        m_status.signatureText = "Runtime missing";
        return;
    }
    m_status.runtimePath = Utf8(m_runtimeFile);
    m_status.runtimeVersion = Version(m_runtimeFile);
    ...
    m_status.signature = signature == ERROR_SUCCESS         ? SignatureState::eSignedValid
                         ...
                         : signature == TRUST_E_NOSIGNATURE ? SignatureState::eUnsigned
                                                            : SignatureState::eUntrusted;
...
void DLSSNRManager::Initialize(UINT width, UINT height) {
    ...
    m_status.backendText = m_runtimeFile.empty()
                               ? "Runtime missing; configure DLSSNR_RUNTIME_FILE or DLSSNR_RUNTIME_PATH"
                               : "Native experimental bridge ready";
#else
    m_status.backend = BackendState::eStubNoSdk;
    m_status.backendText = "Disabled at build time (PATHTRACER_ENABLE_DLSSNR=OFF)";
#endif
}
dlssnr::Tuning DLSSNRManager::Tuning() const {
    ...
    result.modelStyle =
        settings.modelStyle >= 0 && settings.modelStyle < dlssnr::kModelStyleCount ? (uint32_t)settings.modelStyle : 0u;
    result.renderPreset = settings.renderPreset >= 0 && settings.renderPreset < dlssnr::kRenderPresetCount
                              ? (uint32_t)settings.renderPreset
                              : 0u;
    return result;
}
void DLSSNRManager::Fail(const char* stage, uint32_t result) {
    char code[24];
    snprintf(code, sizeof(code), "0x%08X", result);
...
void DLSSNRManager::PrepareFrameGPUIdle() {
    ...
    const bool changed = tuning.intensity != m_tuning.intensity || tuning.localTone != m_tuning.localTone ||
                         tuning.localStructure != m_tuning.localStructure ||
                         tuning.skinStructure != m_tuning.skinStructure || tuning.autoMask != m_tuning.autoMask ||
                         tuning.modelStyle != m_tuning.modelStyle || tuning.renderPreset != m_tuning.renderPreset;
    if (m_featureCreated && (!settings.enabled || changed))
        ReleaseFeature();
    if (settings.enabled && (!m_prevEnabled || changed) && m_status.backend == BackendState::eFailed) {
        ...
        m_status.backendText = "Retrying native bridge";
    }
    if (!settings.enabled || changed || !m_prevEnabled)
        m_forceReset = true;
    if (!settings.enabled && m_status.backend == BackendState::eFeatureActive) {
        m_status.backend = BackendState::eIdle;
        m_status.backendText = "Native bridge ready (disabled)";
    }
    m_tuning = tuning;
...
bool DLSSNRManager::EnsureTextures(ID3D12Device* device) {
    if (m_input && m_output)
        return true;
    auto desc = CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_R8G8B8A8_UNORM, m_width, m_height, 1, 1, 1, 0,
                                             D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    auto heap = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
    HRESULT hr = device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST,
                                                 nullptr, IID_PPV_ARGS(&m_input));
    if (SUCCEEDED(hr))
    ...
    if (FAILED(hr)) {
        m_input.Reset();
        m_output.Reset();
        Fail("CreateTextures", hr);
        return false;
    }
    m_input->SetName(L"DLSSNR_OpaqueSDRInput");
    m_output->SetName(L"DLSSNR_SDRResult");
    m_inputState = D3D12_RESOURCE_STATE_COPY_DEST;
    m_outputState = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
    return true;
}
bool DLSSNRManager::Evaluate(ID3D12GraphicsCommandList* cmd, ID3D12Device* device, ID3D12Resource* color,
    ...
    if (!cmd || !device || !color || !depth || !motion || !rw || !rh) {
        m_forceReset = true;
        return false;
    }
    ...
    if (cd.Format != DXGI_FORMAT_R8G8B8A8_UNORM || cd.Width != m_width || cd.Height != m_height || cd.MipLevels != 1 ||
        subresource >= cd.DepthOrArraySize || dd.Format != DXGI_FORMAT_R32_FLOAT ||
        md.Format != DXGI_FORMAT_R16G16_FLOAT || dd.Width != rw || dd.Height != rh || md.Width != rw ||
        md.Height != rh) {
        Fail("Input contract", 0xBAD00005u);
        return false;
    }
    if (rw != m_guideWidth || rh != m_guideHeight) {
        m_guideWidth = rw;
        m_guideHeight = rh;
        m_forceReset = true;
    }
    ...
    if (!m_context) {
        ...
        if (r != 1u) {
            Fail("Initialize pinned 310.8.0 runtime", r);
            return false;
        }
    }
    ...
    dlssnr::Frame frame;
    frame.color = m_input.Get();
    frame.output = m_output.Get();
    frame.depth = depth;
    frame.motion = motion;
    frame.width = m_width;
    frame.height = m_height;
    frame.guideWidth = rw;
    frame.guideHeight = rh;

    frame.motionScaleX = 1.0f;
    frame.motionScaleY = 1.0f;
    ...
    m_status.backendText = "DLSS 5 Neural Rendering active (experimental native bridge)";
