...
class DLSSNRManager {
  public:
    ...
    void PrepareFrameGPUIdle(); // Call after the previous frame fence.
    void OnDisplayResolution(UINT displayWidth, UINT displayHeight);
    void Shutdown(ID3D12Device* device); // Requires an idle GPU.

    bool Evaluate(ID3D12GraphicsCommandList*, ID3D12Device*, ID3D12Resource* finalColor, UINT finalColorSub,
                  ID3D12Resource* depth, ID3D12Resource* motion, UINT renderWidth, UINT renderHeight);
