...
bool KnownRuntime(const wchar_t* path) {
    constexpr unsigned char expected[32] = {0xe1, 0x6b, 0xcf, 0x15, 0xe1, 0x6e, 0x13, 0xf5, 0x27, 0x49, 0x1c,
                                            0xdf, 0x78, 0x45, 0xb2, 0xfe, 0x65, 0x21, 0xa7, 0x38, 0xd8, 0xf7,
                                            0xc9, 0xc7, 0x21, 0x86, 0x6a, 0x84, 0x96, 0xe1, 0xfc, 0x8e};
    HANDLE file =
    ...
    if (ok)
        ok = BCryptFinishHash(hash, digest, sizeof(digest), 0) >= 0 && memcmp(digest, expected, sizeof(digest)) == 0;
    if (hash)
...
#define NR_GET(T)                                                                                                      \
    ...
    NR_GET(unsigned long long)
    NR_GET(unsigned int) NR_GET(int) NR_GET(float) NR_GET(double) NR_GET(ID3D11Resource*) NR_GET(ID3D12Resource*)
        NR_GET(void*)
#undef NR_GET
            void Reset() override {
        values.clear();
...
void Subrect(Parameters& p, const char* resource, uint32_t w, uint32_t h) {
    ...
    p.Set((prefix + "BaseX").c_str(), 0u);
    p.Set((prefix + "BaseY").c_str(), 0u);
    p.Set((prefix + "Width").c_str(), w);
    p.Set((prefix + "Height").c_str(), h);
}
}
...
uint32_t RoyalNRCreateContext(const wchar_t* runtime, ID3D12Device* device, Context** out) {
    ...
    *out = nullptr;
    ...
    if (result != kSuccess) {
        RoyalNRDestroyContext(ctx.release());
        return result;
    }
    *out = ctx.release();
    return kSuccess;
...
uint32_t RoyalNRCreateFeature(Context* ctx, ID3D12GraphicsCommandList* cmd, uint32_t w, uint32_t h,
                              const Tuning* tuning) {
    if (!ctx || !cmd || !w || !h || !tuning || ctx->feature)
        return kInvalid;
    if (tuning->modelStyle >= kModelStyleCount || tuning->renderPreset >= kRenderPresetCount)
        return kInvalid;
    auto& p = ctx->params;
    p.Set("DLSSNR.Width", static_cast<int>(w));
    p.Set("DLSSNR.Height", static_cast<int>(h));
    ...
    p.Set("DLSSNR.Style", tuning->modelStyle);
...
uint32_t RoyalNREvaluate(Context* ctx, ID3D12GraphicsCommandList* cmd, const Frame* f) {
    ...
    p.Set("DLSSNR.Depth", f->depth);
    p.Set("DLSSNR.MVec", f->motion);
    Subrect(p, "Color", f->width, f->height);
    Subrect(p, "Output", f->width, f->height);
    Subrect(p, "Depth", f->guideWidth, f->guideHeight);
    Subrect(p, "MVec", f->guideWidth, f->guideHeight);
    p.Set("DLSSNR.MVecScaleX", f->motionScaleX);
    p.Set("DLSSNR.MVecScaleY", f->motionScaleY);
...
void RoyalNRDestroyContext(Context* ctx) {
    ...
    if (ctx->initialized) {
        volatile uint32_t result = ctx->shutdown(ctx->device);
        (void)result;
    }
    if (ctx->device)
        ctx->device->Release();
