...
_Use_decl_annotations_ void DXSample::GetHardwareAdapter(IDXGIFactory2* pFactory, IDXGIAdapter1** ppAdapter) {
    ...
    *ppAdapter = nullptr;
    ...
    for (UINT adapterIndex = 0; DXGI_ERROR_NOT_FOUND != pFactory->EnumAdapters1(adapterIndex, &adapter);
        ...
        if (SUCCEEDED(D3D12CreateDevice(adapter.Get(), D3D_FEATURE_LEVEL_12_1, _uuidof(ID3D12Device),
                                        reinterpret_cast<void**>(testDevice.GetAddressOf())))) {
...
_Use_decl_annotations_ void DXSample::ParseCommandLineArgs(WCHAR* argv[], int argc) {
    for (int i = 1; i < argc; ++i) {
        if (_wcsicmp(argv[i], L"-warp") == 0 || _wcsicmp(argv[i], L"/warp") == 0) {
