...
The engine has an **experimental native DLSS 5 NR backend**, disabled by default in the editor. It uses the pinned signed `nvngx_dlssnr.dll` 310.8.0.0. SR, RR and frame generation continue to use the public Streamline SDK.
...
| Pinned runtime folder | Binary runtime bundle | Contains signed `sl.dlss_nr.dll` and `nvngx_dlssnr.dll`, but no SDK headers. |
...
`rdn/PostProcess/DLSSNRBridge.cpp` implements the adapter from the pinned runtime's inspected interface facts.
...
The bridge is named `nvngx.dll_royaltracer_nr.dll` because this runtime checks that its caller module's filename contains `nvngx.dll`. Calls remain inside this bridge, including in optimized builds. It is a separate engine library with no hooks, binary patches, or driver changes.
...
The existing Visual Studio build directory has already been configured with the supplied NR runtime. To reproduce from a developer shell with the project's SDK prerequisites:

```powershell
cmake -S . -B cmake-build-relwithdebinfo-visual-studio `
  -DPATHTRACER_ENABLE_DLSSNR=ON `
  "-DDLSSNR_RUNTIME_FILE=C:/path/to/nvngx_dlssnr.dll"
...
Run the standalone integration regression without rebuilding the renderer:

```powershell
./tests/run_dlssnr_tests.ps1 -RuntimePath "C:/path/to/nvngx_dlssnr.dll"
```
