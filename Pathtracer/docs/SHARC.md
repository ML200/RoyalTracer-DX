...
or update the cache. Disable **Radiance cache** to run the uncached path tracer
with standard bounce limits.

## Research and design

SHaRC uses sparse update paths, temporal resolve and a separate render/query
pass. Records use oriented geometric normals, smooth blending between signed
normal axes, exact key verification, separate surface descriptors and
bilateral geometric/shading-normal and tangent-plane checks. Surface descriptors
and edge-aware support follow the [GI-1.0 report](https://gpuopen.com/download/publications/GPUOpen2022_GI1_0.pdf).
Opposite blanket faces remain separate even when they share a voxel, instance
and material. Nearby parallel sheets can occupy separate records in one
spatial/normal bucket.
...
The standalone GPU runner measures one million cold scattered queries and one
million warm surface queries. Current development-GPU timings are 0.22 ms and
0.04 ms. These are synthetic **lookup-only** measurements; deep training,
visibility rays and uncached fallback paths still require scene-level timing.
SHaRC uses its configured deep cache path budget for training and queries.
