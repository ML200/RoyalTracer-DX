...
- [SkyBakeLayout.h](../shaders/SkyBakeLayout.h) defines a 256×128 sky cache using storage shared with SPMIS only in regular PT mode. Allocate cloud resources independently so they also work under ReSTIR.
...
1. **Reference and timing harness.** Build an independent high-sample volumetric reference using the same density, atmosphere and optical parameters, with genuine higher-order scattering. Existing surface PT alone is not that reference.
