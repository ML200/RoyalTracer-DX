...
September 8, 2026. Input: a 1920x1080, 163-frame advancing-time capture with two lighting samples. Extracted frames show distant horizontal combing and broad red underside illumination changing while the cloud shapes remain nearly stationary. Review artifacts are under `out/cumulus/video-review`.
...
The shadow trace's sample schedule formerly changed when the centre solar ray began hitting the planet. Fixed-distance strata now clip both cloud-shell intervals without moving the whole grid. Atmospheric and cloud direct illumination use the visible disk centroid for grazing transmittance/shadow directions, with disk fraction separately controlling energy. The finite-radius visibility width follows the geometric horizon factor. The centroid approximation and fixed shadow schedule are implementation choices here, not claims of a full solar-disk integral.

The approved material and normal functions are unchanged. No texture allocations, bindings, history buffers, settings layout or executable changes were added.
