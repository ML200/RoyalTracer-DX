...
void main(uint3 id:SV_DispatchThreadID)
{
    ...
    CumulusResult r=IntegrateCumulus(V,sun.dirWS,-1.0f,min(24u,(uint)cloudReflectionSteps),sampleJitter,false,pixelAngle);
    ...
    // Kilometres retain range in fp16. The guide consumer converts to metres only when needed.
    float opacity=1.0f-r.cloudT;
    // Premultiply guide distances before filtering: clear texels must not pull cloud depth toward zero.
    g_cumulusEnvironmentOut[uint3(p,1)]=float4(float2(r.depthKm,r.deviationKm)*opacity,opacity,0);
}
