...
int main() {
    ...
    if (manager.GetStatus().backend != DLSSNRManager::BackendState::eStubNoSdk ||
        manager.Evaluate(nullptr, nullptr, nullptr, 0, nullptr, nullptr, 1280, 720))
        return 1;
...
void Check(HRESULT hr) {
    if (FAILED(hr)) {
        char error[40];
        sprintf_s(error, "D3D12 0x%08X", (unsigned)hr);
...
struct Inputs {
    ...
    Inputs(Gpu& g, UINT w, UINT h, UINT renderW, UINT renderH) : width(w), height(h), rw(renderW), rh(renderH) {
        ...
        Check(g.device->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&gpu)));
        ...
        g.Begin();
        ID3D12DescriptorHeap* heaps[] = {gpu.Get()};
        g.list->SetDescriptorHeaps(1, heaps);
        for (UINT i = 0; i < 3; ++i) {
            auto desc = CD3DX12_RESOURCE_DESC::Tex2D(formats[i], i ? rw : width, i ? rh : height, 1, 1, 1, 0,
                                                     D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
            Check(g.device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc,
                                                    D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
                                                    IID_PPV_ARGS(textures[i]->ReleaseAndGetAddressOf())));
            ...
            g.list->ClearUnorderedAccessViewFloat(
                CD3DX12_GPU_DESCRIPTOR_HANDLE(gpu->GetGPUDescriptorHandleForHeapStart(), i, stride), cpuHandle,
                textures[i]->Get(), clear[i], 0, nullptr);
        }
        auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(color.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
        ...
        auto buffer = CD3DX12_RESOURCE_DESC::Buffer(totalBytes);
        Check(g.device->CreateCommittedResource(&readHeap, D3D12_HEAP_FLAG_NONE, &buffer,
    ...
    void SetMotion(Gpu& g, float x, float y) {
        ...
        g.list->ClearUnorderedAccessViewFloat(
            ...
            CD3DX12_CPU_DESCRIPTOR_HANDLE(cpu->GetCPUDescriptorHandleForHeapStart(), 2, stride), motion.Get(), value, 0,
            nullptr);
    ...
    uint64_t Evaluate(Gpu& g, DLSSNRManager& manager, bool expected, const std::function<void()>& beforeNR = {}) {
        ...
        if (beforeNR)
            beforeNR();
        bool evaluated =
            manager.Evaluate(g.list.Get(), g.device.Get(), color.Get(), 0, depth.Get(), motion.Get(), rw, rh);
        ...
        if (evaluated != expected)
            fprintf(stderr, "NR status: %s / %s\n", manager.GetStatus().backendText.c_str(),
                    manager.GetStatus().lastResult.c_str());
        ...
        if (!expected && !beforeNR)
            Require(sum == uint64_t(width) * height * (51 + 102 + 153), "Fallback modified source color");
        printf("%ux%u guides %ux%u %s sum=%llu\n", width, height, rw, rh, evaluated ? "NR" : "fallback",
...
void CheckRRAndNR(Gpu& g, const wchar_t* runtime) {
    ...
    DLSSManager rr;
    rr.mode = sl::DLSSMode::eMaxQuality;
    rr.CreateResources(g.device.Get(), 1280, 720);
    Inputs input(g, 1280, 720, rr.RenderWidth(), rr.RenderHeight());
    input.depth = rr.Depth();
    input.motion = rr.MVec();
    DLSSNRManager nr;
    nr.Initialize(1280, 720);
    nr.settings.enabled = true;
    ComPtr<ID3D12DescriptorHeap> cpu, gpu;
    D3D12_DESCRIPTOR_HEAP_DESC hd{};
    hd.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    ...
    Check(g.device->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&gpu)));
    const UINT stride = g.device->GetDescriptorHandleIncrementSize(hd.Type);
    ID3D12Resource* guides[] = {
        rr.Input(),     rr.Depth(),    rr.MVec(),        rr.Normals(),      rr.DiffuseAlbedo(),    rr.SpecularAlbedo(),
    ...
    for (UINT i = 0; i < _countof(guides); ++i) {
        ...
        g.device->CreateUnorderedAccessView(
            guides[i], nullptr, &view,
            CD3DX12_CPU_DESCRIPTOR_HANDLE(gpu->GetCPUDescriptorHandleForHeapStart(), i, stride));
        ...
        if (i == 4)
            clear[0] = clear[1] = clear[2] = .5f;
        if (i == 5)
            clear[0] = clear[1] = clear[2] = .04f;
        if (i == 6)
            clear[0] = .6f;
        g.list->ClearUnorderedAccessViewFloat(
            CD3DX12_GPU_DESCRIPTOR_HANDLE(gpu->GetGPUDescriptorHandleForHeapStart(), i, stride), c, guides[i], clear, 0,
            nullptr);
    }
    auto ready = CD3DX12_RESOURCE_BARRIER::UAV(nullptr);
    g.list->ResourceBarrier(1, &ready);
    g.Submit();
    D3D12_SHADER_RESOURCE_VIEW_DESC srv{};
    srv.Format = rr.Output()->GetDesc().Format;
    srv.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
    srv.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
    srv.Texture2D.MipLevels = 1;
    g.device->CreateShaderResourceView(
        rr.Output(), &srv, CD3DX12_CPU_DESCRIPTOR_HANDLE(gpu->GetCPUDescriptorHandleForHeapStart(), 12, stride));
    D3D12_UNORDERED_ACCESS_VIEW_DESC uav{};
    uav.Format = input.color->GetDesc().Format;
    uav.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;
    g.device->CreateUnorderedAccessView(
        input.color.Get(), nullptr, &uav,
        CD3DX12_CPU_DESCRIPTOR_HANDLE(gpu->GetCPUDescriptorHandleForHeapStart(), 13, stride));
    ...
    Check(D3D12SerializeRootSignature(&signature, D3D_ROOT_SIGNATURE_VERSION_1, &serialized, &error));
    ComPtr<ID3D12RootSignature> root;
    Check(g.device->CreateRootSignature(0, serialized->GetBufferPointer(), serialized->GetBufferSize(),
                                        IID_PPV_ARGS(&root)));
    ...
    Check(g.device->CreateComputePipelineState(&pd, IID_PPV_ARGS(&pso)));
    sl::FrameToken* token = nullptr;
    sl::ViewportHandle viewport(17);
    uint32_t frame = 0;
    auto reconstruct = [&]() {
        Require(slGetNewFrameToken(token, nullptr) == sl::Result::eOk, "Frame token failed");
        const auto view = XMMatrixIdentity(),
                   projection = XMMatrixPerspectiveFovRH(XMConvertToRadians(60), 1280.f / 720.f, .1f, 10000.f);
        rr.Evaluate(g.list.Get(), g.device.Get(), *token, viewport, 1280.f / 720.f, view, view, projection, 0, 0,
                    ++frame, 60, .1f, 10000);
        if (g.messages)
            g.messages->SetBreakOnSeverity(D3D12_MESSAGE_SEVERITY_ERROR, FALSE);
        Require(rr.LastEvaluationSucceeded(), "RR evaluation failed alongside NR");
        if (rr.LastEvaluationReset())
            nr.ForceReset();
        D3D12_RESOURCE_BARRIER barriers[] = {
            CD3DX12_RESOURCE_BARRIER::Transition(rr.Output(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                                                 D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE),
            CD3DX12_RESOURCE_BARRIER::Transition(input.color.Get(), D3D12_RESOURCE_STATE_COPY_SOURCE,
                                                 D3D12_RESOURCE_STATE_UNORDERED_ACCESS)};
        g.list->ResourceBarrier(2, barriers);
        g.list->SetDescriptorHeaps(1, heaps);
        g.list->SetComputeRootSignature(root.Get());
        g.list->SetPipelineState(pso.Get());
        for (UINT i = 0; i < 2; ++i)
            g.list->SetComputeRootDescriptorTable(
                i, CD3DX12_GPU_DESCRIPTOR_HANDLE(gpu->GetGPUDescriptorHandleForHeapStart(), 12 + i, stride));
        g.list->Dispatch(160, 90, 1);
        for (auto& b : barriers)
            std::swap(b.Transition.StateBefore, b.Transition.StateAfter);
        g.list->ResourceBarrier(2, barriers);
    };
    input.Evaluate(g, nr, false, reconstruct);
    ...
    nr.Shutdown(g.device.Get());
    nr.settings.enabled = false;
    input.Evaluate(g, nr, false, reconstruct);
    Require(slFreeResources(sl::kFeatureDLSS_RR, viewport) == sl::Result::eOk, "RR resource release failed");
    fprintf(stderr, "PASS: engine RR -> opaque SDR conversion -> NR on the same command list; RR reset propagation and "
                    "RR after NR shutdown.\n");
}
}

int wmain(int argc, wchar_t** argv) {
    if (argc != 2) {
        puts("Usage: DLSSNRGpuTests.exe absolute-path-to-nvngx_dlssnr.dll");
        return 2;
    }
    try {
        ...
        Gpu g;
        Require(slSetD3DDevice(g.device.Get()) == sl::Result::eOk, "Streamline device failed");
        LUID luid = g.device->GetAdapterLuid();
        sl::AdapterInfo ai{};
        ai.deviceLUID = reinterpret_cast<uint8_t*>(&luid);
        ai.deviceLUIDSizeInBytes = sizeof(luid);
        for (auto feature : features)
            Require(slIsFeatureSupported(feature, ai) == sl::Result::eOk, "SR/RR support regressed");
        DLSSNRManager manager;
        manager.Initialize(1920, 1080);
        ...
        for (int style = 0; style < dlssnr::kModelStyleCount; ++style) {
            for (int preset = 0; preset < dlssnr::kRenderPresetCount; ++preset) {
                if (style == 0 && preset == 0)
                    continue;
                manager.settings.modelStyle = style;
                manager.settings.renderPreset = preset;
                inputs.Evaluate(g, manager, false);
                const auto selected = inputs.Evaluate(g, manager, true);
                if (preset == 0)
                    Require(selected != first, "Model-style selection did not affect NR output");
                manager.ForceReset();
                Require(inputs.Evaluate(g, manager, true) == selected,
                        "Selected NR model/preset reset was not deterministic");
                printf("NR selection: style=%d preset=%d sum=%llu\n", style, preset, (unsigned long long)selected);
            }
        }
        manager.settings.modelStyle = 0;
        manager.settings.renderPreset = 0;
        inputs.Evaluate(g, manager, false);
        ...
        manager.settings.renderPreset = 99;
        inputs.Evaluate(g, manager, true);
        manager.settings.modelStyle = 0;
        manager.settings.renderPreset = 0;
        ...
        inputs.Evaluate(g, manager, false);
        manager.settings.modelStyle = 2;
        manager.settings.renderPreset = 3;
        inputs.Evaluate(g, manager, false);
        manager.settings.enabled = true;
        inputs.Evaluate(g, manager, false);
        inputs.Evaluate(g, manager, true);
        ...
        resized.Evaluate(g, manager, false);
        resized.Evaluate(g, manager, true);
        Inputs lowRes(g, 1280, 720, 854, 480);
        lowRes.Evaluate(g, manager, true);
        g.Begin();
        Require(!manager.Evaluate(g.list.Get(), g.device.Get(), lowRes.color.Get(), 0, lowRes.depth.Get(),
                                  lowRes.motion.Get(), 999, 480),
                "Invalid guide dimensions accepted");
        g.Submit();
        manager.settings.modelStyle = 1;
        lowRes.Evaluate(g, manager, false);
        lowRes.Evaluate(g, manager, true);
        manager.Shutdown(g.device.Get());
        SetEnvironmentVariableW(L"DLSSNR_RUNTIME_PATH", L"Z:\\missing-test-runtime\\nvngx_dlssnr.dll");
        DLSSNRManager missing;
        missing.Initialize(1280, 720);
        missing.settings.enabled = true;
        lowRes.Evaluate(g, missing, false);
        missing.Shutdown(g.device.Get());
        dlssnr::Context* invalid = nullptr;
        Require(dlssnr::RoyalNRCreateContext(L"Z:\\missing-test-runtime.dll", g.device.Get(), &invalid) != 1 &&
                    invalid == nullptr,
                "Unknown runtime accepted");
        wchar_t wrongRuntime[32768]{};
        GetModuleFileNameW(nullptr, wrongRuntime, _countof(wrongRuntime));
        Require(dlssnr::RoyalNRCreateContext(wrongRuntime, g.device.Get(), &invalid) != 1 && invalid == nullptr,
                "Runtime hash mismatch accepted");
        CheckRRAndNR(g, argv[1]);
        fprintf(stderr, "PASS: NR rendering checks; shutting down Streamline.\n");
        Require(slShutdown() == sl::Result::eOk, "Streamline shutdown failed");
        fprintf(stderr,
                "PASS: SR/RR initialization alongside NR, NR GPU output, nonzero motion, all model styles/presets, "
                "default restoration, deterministic reset, tuning, toggles, output/guide resize, invalid inputs, "
                "missing runtime; D3D12 debug validation %s.\n",
