param([string]$OutputDirectory = (Join-Path $env:TEMP 'RoyalTracer-SharcTests'))
$ErrorActionPreference = 'Stop'
$projectRoot = Split-Path $PSScriptRoot -Parent
New-Item -ItemType Directory -Path $OutputDirectory -Force | Out-Null
$dxc = Join-Path $projectRoot 'include/dxc.exe'
foreach ($entry in @('prepare', 'fill', 'resolve', 'query', 'eraseTop', 'benchmark', 'guideFill', 'guideQuery')) {
    $readOnly = if ($entry -in @('query', 'benchmark')) { @('-D', 'SHARC_READ_ONLY=1') } else { @() }
    & $dxc @readOnly -T cs_6_6 -E $entry -HV 2021 -enable-16bit-types -O3 -WX `
...
& cl.exe /nologo /std:c++17 /EHsc /O2 (Join-Path $PSScriptRoot 'SharcGpuTests.cpp') `
