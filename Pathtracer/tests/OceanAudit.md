...
| Optics | `Material_Common_v8.hlsli:CalculateAbsorptionThroughput`, `Material_Decoder_v8.hlsli`, `OceanOptics.hlsli` | Existing glass absorption retained. Water bypasses solid SSS and uses its controls for segment extinction and sun/sky single scattering. Water-only reflection proposal has a 70% floor with matching PDFs; other materials retain their original rule. Water membership follows surface crossings; camera-inside transport is supported. Nested media and multiple scattering are not implemented. |
