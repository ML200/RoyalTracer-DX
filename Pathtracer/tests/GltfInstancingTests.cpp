...
static void TestGpuNormals(Scene& scene, ID3D12Device* device, const fs::path& shaderPath) {
    ...
    for (int instance = 0; instance < 2; ++instance) {
        ...
        Near(values[3 * instance].w, 0.0f, "GPU light record is not shared between instances");
...
static void TestScene(const fs::path& dir, ID3D12Device* device) {
    ...
    Require(scene.emissiveTriangles.size() == 1, "Shared emissive geometry was duplicated per instance");
    Require(scene.meshLightBase[0] == 0 && scene.triToLightId.size() == 2, "Per-mesh light table sized wrong");
    Require(scene.triToLightId[0] == 0 && scene.triToLightId[1] == UINT_MAX, "Non-emissive triangle became a light");
    Require(scene.lightInstances.size() == 2 && scene.instanceLightSlot[0] == 0 && scene.instanceLightSlot[1] == 1 &&
        scene.lightInstances[1].instanceID == 1 && scene.lightInstances[1].meshID == 0, "Light slots not assigned per instance");
    {
        std::vector<InstanceXformCPU> xf(2);
        for (int i = 0; i < 2; ++i) XMStoreFloat4x4(&xf[i].objectToWorld, scene.instances[i].worldTransform);
        lt::LightTreeBuilder builder;
        builder.Build(scene.emissiveTriangles, scene.lightInstances, xf);
        const auto& tlas = builder.GetCpuTLASNodes();
        Require(builder.BLASCount() == 1 && builder.SlotCount() == 2 && !tlas.empty() && tlas[0].childCount == 2,
            "Instanced light tree has the wrong shape");
        const auto& leafA = tlas[tlas[0].firstChild];
        const auto& leafB = tlas[tlas[0].firstChild + 1];
        Require(leafA.childCount == 0 && leafB.childCount == 0 && leafA.slot + leafB.slot == 1, "TLAS leaves do not name the two slots");
        Near(builder.SlotRecords()[1].powerScale / builder.SlotRecords()[0].powerScale,
            lt::areaScale(xf[1].objectToWorld) / lt::areaScale(xf[0].objectToWorld), "Slot power scale does not follow the instance transform");
    }
