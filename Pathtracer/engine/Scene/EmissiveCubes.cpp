...
void EmissiveCubes::GenCube(float h, UINT matIdx, std::vector<Vertex>& V, std::vector<UINT>& I) {
    auto quad = [&](XMFLOAT3 p0, XMFLOAT3 p1, XMFLOAT3 p2, XMFLOAT3 p3, XMFLOAT3 n) {
        UINT b = (UINT)V.size();
        float m = (float)matIdx;
        V.push_back({p0, {n.x, n.y, n.z, m}, {0, 0}});
        V.push_back({p1, {n.x, n.y, n.z, m}, {1, 0}});
        V.push_back({p2, {n.x, n.y, n.z, m}, {1, 1}});
        V.push_back({p3, {n.x, n.y, n.z, m}, {0, 1}});
        I.push_back(b);
        I.push_back(b + 1);
        I.push_back(b + 2);
        I.push_back(b);
...
void EmissiveCubes::Init(const Params& params, SceneManager& sm, Renderer& renderer) {
    ...
    firstMat.Kd = {1, 1, 1, 1};
    firstMat.Ke = {1, 1, 1};
    firstMat.Pr_Pm_Ps_Pc = {0.5f, 0, 0, 0};
    firstMat.Ni = 1;
    UINT baseMeshIdx = renderer.CreateProceduralMesh(sharedVerts, sharedIndices, firstMat);
    renderer.GetScene().SetMeshLightClass(baseMeshIdx, Scene::LightClassCubes);
    ...
    for (int i = 0; i < params.count; ++i) {
        ...
        if (isEmissive) {
            ...
            mat.Kd = {r, g, b, 1};
            mat.Ke = {0, 0, 0};
            float roughness = 0.05f + dH(m_rng) * 0.9f;
            float metalness = (dH(m_rng) < 0.3f) ? 1.0f : 0.0f;
            mat.Pr_Pm_Ps_Pc = {roughness, metalness, 0, 0};
        }

        UINT meshIdx = (i == 0) ? baseMeshIdx : renderer.CreateMeshInstance(baseMeshIdx, mat);
        renderer.GetScene().SetMeshLightClass(meshIdx, Scene::LightClassCubes);

        Transform t;
        t.position = {dX(m_rng), dY(m_rng), dZ(m_rng)};
        uint32_t id = sm.Instantiate(meshIdx, t);

        float spd = dS(m_rng);
        XMFLOAT3 dir = {dD(m_rng), dD(m_rng) * 0.3f, dD(m_rng)};
        ...
        CubeInstance ci;
        ci.objectId = id;
        ci.velocity = {dir.x * spd, dir.y * spd, dir.z * spd};
        ci.targetVelocity = ci.velocity;
        ci.changeTimer = dH(m_rng) * 3;
...
void EmissiveCubes::Update(float dt, SceneManager& sm) {
    ...
    for (auto& c : m_cubes) {
        auto* obj = sm.Get(c.objectId);
        if (!obj)
            continue;
        c.changeTimer -= dt;
        if (c.changeTimer <= 0) {
            float spd = dS(m_rng);
            XMFLOAT3 d = {dD(m_rng), dD(m_rng) * 0.3f, dD(m_rng)};
        ...
        p.y += c.velocity.y * dt;
        p.z += c.velocity.z * dt;
        if (p.x < m_params.spawnMin.x) {
            p.x = m_params.spawnMin.x;
            c.velocity.x = fabsf(c.velocity.x);
        }
        if (p.x > m_params.spawnMax.x) {
            p.x = m_params.spawnMax.x;
            c.velocity.x = -fabsf(c.velocity.x);
        }
        if (p.y < m_params.spawnMin.y) {
            p.y = m_params.spawnMin.y;
            c.velocity.y = fabsf(c.velocity.y);
        }
        if (p.y > m_params.spawnMax.y) {
            p.y = m_params.spawnMax.y;
            c.velocity.y = -fabsf(c.velocity.y);
        }
        if (p.z < m_params.spawnMin.z) {
            p.z = m_params.spawnMin.z;
            c.velocity.z = fabsf(c.velocity.z);
        }
        if (p.z > m_params.spawnMax.z) {
            p.z = m_params.spawnMax.z;
            c.velocity.z = -fabsf(c.velocity.z);
        }
