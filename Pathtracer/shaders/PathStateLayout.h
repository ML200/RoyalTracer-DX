...
// Path-state buffer [plane][pixel], bytes per pixel. Shared with the host (PS_PATH_STATE_BYTES).
...
// Deferred records (PtDefer_v8.hlsli).
...
// Reuse shift scratch aliases the vertex record, dead after the sample loop.
#define PS_LITE_SHIFT_PLANE  DV_VERT_PLANE
#define PS_LITE_SHIFT_BYTES  DV_VERT_BYTES

// Per training lane (<= 1/4 of pixels): two 32-byte guide roots, 48-byte spill.
