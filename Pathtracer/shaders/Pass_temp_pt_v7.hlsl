    ...
    if(all(sdata.L1 < EPSILON)){
        ...
        if(tempPixelIdx != 0xFFFFFFFF){
            ...
            bool candidateAcceptedDI =
                ...
                !RejectNormal_GI(sdata.n1, sdata_r.n1, 0.3f) &&
                (!RejectDistance_GI(sdata.x1, sdata_r.x1, sdata.n1, 0.1f) || length(sdata.x1 - sdata_r.x1) < 0.01f)  &&
                //!(rdi.W_gi > 1.0f/1e-10 || rdi.W_gi < 1e-10) &&
                //!RejectLength_GI(rdi.x2_gi, rdi.n2_gi, sdata.x1, sdata_r.x1, 0.05f) &&
