/*
    ...
    Layout (per pixel):
        ...
        Offset  8: barycentrics                                  [half2 -> uint32]
        Offset 12: etai / etat                                   [half2 -> uint32]
        Offset 16: n1_g (geometric normal, object space, packed) [uint32]
        Offset 20: n1_s (shading normal, object space, packed)   [uint32]
        Offset 24: uv (texture coordinates)                      [half2 -> uint32]
    Total: 28 bytes.
...
{
    buf.Store(pixelBaseAddr_SD(pixelIdx) + 8u, PackFloat2x16(bary.x, bary.y));
}

void store_etai_etat(RWByteAddressBuffer buf, uint pixelIdx, float etai, float etat)
{
    buf.Store(pixelBaseAddr_SD(pixelIdx) + 12u, PackFloat2x16(etai, etat));
}
...
void store_n1_g_world(RWByteAddressBuffer buf, uint pixelIdx, float3 n1g_world, uint instID)
{
    ...
    buf.Store(pixelBaseAddr_SD(pixelIdx) + 16u, PackNormal(n1g_obj));
}
...
{
    buf.Store(pixelBaseAddr_SD(pixelIdx) + 24u, PackFloat2x16(uv.x, uv.y));
}
...
void store_sky(RWByteAddressBuffer buf, uint pixelIdx)
{
    ...
    buf.Store4(base, uint4(0xFFFFu | 0x80000000u, 0u, 0u, PackFloat2x16(1.0f, 1.0f)));
    buf.Store3(base + 16u, uint3(0u, 0u, 0u));
}
...
{
    uint packed = buf.Load(pixelBaseAddr_SD(pixelIdx) + 8u);
    float a, b;
...
{
    return f16tof32_custom(buf.Load(pixelBaseAddr_SD(pixelIdx) + 12u) & 0xFFFFu);
}

float load_etat(RWByteAddressBuffer buf, uint pixelIdx)
{
    return f16tof32_custom(buf.Load(pixelBaseAddr_SD(pixelIdx) + 12u) >> 16);
}
...
{
    uint packed = buf.Load(pixelBaseAddr_SD(pixelIdx) + 24u);
    float a, b;
    UnpackFloat2x16(packed, a, b);
    return float2(a, b);
}
...
void copySampleData(RWByteAddressBuffer dst, RWByteAddressBuffer src, uint pixelIdx)
{
    ...
    dst.Store3(base + 16u, src.Load3(base + 16u));
}
