...
{
    // 1. Use your proven coordinate function which correctly applies the -0.5f pixel center offset
    float2 px = GetLastFramePixelCoordinates_Float(worldPos, prevView, prevProjection, resolution, objID);

    // Early out on failure (Out of bounds or invalid clip space)
    if (px.x < 0.0f)
    {
    ...
    // 2. Standard bilinear interpolation math
    int2   pix00  = int2(floor(px));
    float2 frac   = px - floor(px); // Strictly between [0.0, 1.0)

    // All 4 possible neighbors
    const int2 offs[4] = { int2(0,0), int2(1,0), int2(0,1), int2(1,1) };

    // Pure bilinear weights based on sub-pixel location
    float basis[4] =
    {
        (1.0f - frac.x) * (1.0f - frac.y), // Top-Left
        frac.x          * (1.0f - frac.y), // Top-Right
        (1.0f - frac.x) * frac.y,          // Bottom-Left
        frac.x          * frac.y           // Bottom-Right
    };
