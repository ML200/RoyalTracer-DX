...
// -----------------------------------------------------------------------------
//  Inline closest-hit replacement
// -----------------------------------------------------------------------------
[forceinline]          // keep it in the caller for performance
bool TraceRayInlineWithPayload(
        in  RayDesc   ray,          // world-space ray you would have passed to TraceRay()
        out HitInfo   payload)      // filled exactly like your CHS did
{
    // zero-initialise so the caller can safely read on a miss
    payload = (HitInfo)0;

    // ---------------------------------------------------------------------
    // 1)  Launch an inline query that stops as soon as it finds the
    //     first triangle (identical to your CHS compile flags)
    // ---------------------------------------------------------------------
    RayQuery< RAY_FLAG_ACCEPT_FIRST_HIT_AND_END_SEARCH > rq;

    rq.TraceRayInline(SceneBVH,          // TLAS  (same SRV you already bind)
                      /*dynamicFlags*/RAY_FLAG_NONE,
                      /*mask*/        0xFF,
                      ray);

    // Drive traversal to completion
    rq.Proceed();      // one call is enough because we set ACCEPT_FIRST_HIT

    if (rq.CommittedStatus() != COMMITTED_TRIANGLE_HIT)
        return false;  // ← miss: payload stays zero-initialised

    // ---------------------------------------------------------------------
    // 2)  Everything that used to be in your closest-hit shader
    //     (copy-pasted & slightly adapted)
    // ---------------------------------------------------------------------
    uint   instID   = rq.CommittedInstanceIndex();
    ...
    uint   vertId   = primID * 3;

    // --- fetch per-vertex data ------------------------------------------------
    float3 v0 = BTriVertex[indices[vertId    ]].vertex;
    float3 v1 = BTriVertex[indices[vertId + 1]].vertex;
    float3 v2 = BTriVertex[indices[vertId + 2]].vertex;

    // material ID is encoded exactly like in your CHS
    uint materialID = materialIDs[vertId + BTriVertex[indices[vertId]].normal.w];

    // --- flat and (optional) smooth normals -----------------------------------
    float3 e1    = v1 - v0;
    float3 e2    = v2 - v0;
    float3 crossA= cross(e1, e2);
    float3 flatN = normalize(crossA);

    float3 smoothN = 0;
    [unroll]
    for (int i = 0; i < 3; ++i)
    {
        float3 n = BTriVertex[indices[vertId + i]].normal.xyz;
        smoothN += (all(n == 0) ? flatN : n) * bary[i];
    }
    float3 normal = (length(smoothN) > 1e-4) ? normalize(smoothN) : flatN;
    // transform to world space
    normal = normalize(
                mul(instanceProps[instID].objectToWorldNormal,
                    float4(normal, 0.0)).xyz);

    // --- area (exactly what CHS stored) ---------------------------------------
    float  area   = 0.5f * length(crossA);

    // --- world-space hit position --------------------------------------------
    float3 hitPos = ray.Origin + ray.Direction * rq.CommittedRayT();

    // ---------------------------------------------------------------------
    // 3)  Fill the payload exactly like your CHS did
    // ---------------------------------------------------------------------
    payload.objID       = instID;
    payload.area        = area;
