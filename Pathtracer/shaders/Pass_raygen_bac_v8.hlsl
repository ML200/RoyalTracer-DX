...
void Pass_raygen_v8()
{
    ...
    gOutput[uint3(DispatchRaysIndex().xy, 3)] = (float4)0;
    ...
    for (int depth = 0; depth < MAX_BOUNCES; ++depth)
    {
        ...
        if (!hitObj.IsHit())
        {
            ...
            gScratchPing[uint3(pix, 1)] += float4(T * EvalMissState(rayDir), 0);
            if(depth == 1){
                uint idx4 = MapPixelID(float2(DispatchRaysDimensions().xy), DispatchRaysIndex().xy);
                float p_hat = GetPHat(T * EvalMissState(rayDir) * prev_pdf); // We need to remove the previous pdf; Cancel it my multiplying with it
                float wi = p_hat / prev_pdf;
                float3 dir = rayDir;
                bool update = UpdateReservoirDI_Infinite(g_Reservoirs_current_di, idx4, wi, dir, EvalMissState(rayDir), 0xFFFFFFFFu, seed);
        ...
        //gOutput[uint3(DispatchRaysIndex().xy, 3)] = float4(s, 1.0f);
    ...
    //gOutput[uint3(DispatchRaysIndex().xy, 3)] = gScratchPing[uint3(DispatchRaysIndex().xy, 3)];
}
