...
void main(uint3 tid : SV_DispatchThreadID)
{
    ...
    if(all(sdata.L1 < EPSILON)){
        ...
        float mis_c = PairwiseMIS_Canonical_Spat_DI(M_sum, p_c, M_c, nIds, sdata.x1, rdi.x2_di, rdi.n2_di, rdi.L2_di, rdi.objID_di);
        //debug += mis_c;
        ...
        for(int i = 0; i < SPAT_COUNT_MAX_DI; i++){
            if(nIds[i] != 0xFFFFFFFF){
                ...
                float mis_n = PairwiseMIS_Neighbor_Spat_DI(M_sum, M_c, min(SPAT_MCAP_DI ,rdi_r.M_di), p_c, p_hat_from, nIds[i], rdi_r.x2_di, rdi_r.n2_di, rdi_r.L2_di, rdi_r.objID_di);
                //debug += mis_n;
