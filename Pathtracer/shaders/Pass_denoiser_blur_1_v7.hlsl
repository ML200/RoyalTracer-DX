...
// ─── Spatial à-trous filter – Pass 0 (stride = 1) with hit‑valid mask ────────
// Adds the same linear‑depth + normal rejection used in the temporal stage so
// neighbours across sharp depth/normal transitions contribute **zero** weight.
// ----------------------------------------------------------------------------
[shader("raygeneration")]
void Pass_denoiser_blur_1_v7()
{
    uint2  launch = DispatchRaysIndex().xy;
    float2 dims   = DispatchRaysDimensions().xy;

    // ── Constants -----------------------------------------------------------
    const int   STRIDE = 1;   // first à‑trous step (stride 1)
    const int   K      = 1;   // 3×3 kernel radius
