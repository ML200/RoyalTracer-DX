...
[shader("closesthit")] void ClosestHit(inout HitInfo payload,
    ...
    if(recieveDir){
        if(payload.util.y != 0)
            direct  = float3(10,10,10) * payload.colorAndDistance.xyz * attenuation * max(0.f, dot(normal, centerLightDir)) * factor * brdf * materials[materialID].Pr_Pm_Ps_Pc.x * materials[materialID].Pr_Pm_Ps_Pc.x * materials[materialID].Kd;
        else
            direct  = float3(10,10,10) * payload.colorAndDistance.xyz * attenuation * max(0.f, dot(normal, centerLightDir)) * factor * brdf * materials[materialID].Kd;
    }
