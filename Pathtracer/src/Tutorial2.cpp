...
#include "./Renderer/inc/Application.h"
#include "./Renderer/inc/CommandQueue.h"
#include "./Renderer/inc/Helpers.h"
        ...
        D3D12_RESOURCE_FLAGS flags)
{
    ...
    // You're creating a default heap resource twice. The second creation should be an upload heap.
    ...
    // Create an intermediate upload resource only if bufferData is provided.
    if (bufferData)
    {
        // This should be D3D12_HEAP_TYPE_UPLOAD for the intermediate buffer.
        heapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
        // Resource description can be the default buffer without any flags for upload heaps.
        ...
        ThrowIfFailed(device->CreateCommittedResource(
                ...
                D3D12_HEAP_FLAG_NONE,
                &resourceDesc,
                D3D12_RESOURCE_STATE_GENERIC_READ, // Upload heaps can remain in GENERIC_READ state.
                nullptr,
                IID_PPV_ARGS(pIntermediateResource))); // This should point to the intermediate resource.

        // Prepare the data to be uploaded to the GPU.
        D3D12_SUBRESOURCE_DATA subresourceData = {};
        subresourceData.pData = bufferData;
        subresourceData.RowPitch = bufferSize;
        subresourceData.SlicePitch = bufferSize; // For buffers, RowPitch and SlicePitch are usually the same.

        // Copy data to the intermediate upload heap and then schedule a copy from the upload heap to the default heap.
        UpdateSubresources<1>(commandList.Get(), *pDestinationResource, *pIntermediateResource, 0, 0, 1, &subresourceData);

        // Transition the destination buffer to the appropriate state if necessary.
        // This may be required if the buffer will be used in a different state immediately after.
    }
}
...
// Create named instances of heap properties and resource description
        auto heapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
